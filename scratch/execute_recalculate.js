...
async function runRecalculationForCompany(companyId) {
  console.log(`\n========================================`);
  console.log(`Executing recalculation for company: ${companyId}`);
  console.log(`========================================`);
  ...
  if (!suspenseAccount) {
    ...
    console.log('Created Suspense account (9999)');
  }
  ...
  if (deletedEntries.count > 0) {
    console.log(`Deleted ${deletedEntries.count} previous OPENING_BALANCE_ADJUSTMENT entries`);
  }
  ...
  // Fix stored totals on journal entries
  ...
  for (const entry of journalEntries) {
    ...
    if (
      ...
      console.log(`${entry.entryNumber}: stored totals corrected (Dr=${lineDebit.toFixed(2)}, Cr=${lineCredit.toFixed(2)})`);
      entriesFixed++;
    }
    ...
    if (diff > 0.005) {
      ...
      console.log(`${entry.entryNumber}: was unbalanced by ₹${diff.toFixed(2)} — Suspense line added`);
      balancingLinesAdded++;
      entriesFixed++;
    }
  }

  // Recalculate ChartOfAccount from journal lines
  ...
  // Calculate Trial Balance discrepancy before Suspense adjustment
  let trialDr = 0;
  let trialCr = 0;
  ...
  const trialDiff = trialDr - trialCr;

  // Build correcting journal entry lines
  const correctingLines = [];
  ...
  if (loansReceivableAccount) {
    ...
    const diffLoansReceivable = 0 - journalLoansReceivable;
  ...
  if (correctingLines.length > 0) {
    ...
    console.log(`Created correcting entry: ${nextNumber}`);
  } else {
    console.log('Trial Balance is already balanced ✓');
  }

  // Update balances in ChartOfAccount table
  ...
  for (const acc of finalAccounts) {
    ...
    if (Math.abs(newBalance - (acc.currentBalance || 0)) > 0.005) {
      ...
      console.log(`Updated ${acc.accountCode} ${acc.accountName}: ${acc.currentBalance} -> ${newBalance}`);
    }
  }

  console.log(`Recalculation execution completed successfully for company: ${companyId}`);
}
...
async function main() {
  ...
  await runRecalculationForCompany(c1);
  await runRecalculationForCompany(c3);
}
