...
const accrual_helper_1 = require("@/lib/accrual-helper");
/**
...
async function GET(request) {
    try {
        ...
        await (0, accrual_helper_1.performOnDemandAccrual)(companyId);
        ...
        return server_1.NextResponse.json({ success: false, error: 'Missing parameters' }, { status: 400 });
    }
    catch (error) {
        console.error('[Personal Ledger] Error:', error);
        return server_1.NextResponse.json({ success: false, error: 'Failed to fetch personal ledger', details: error.message }, { status: 500 });
    }
}
// ─────────────────────────────────────────────────────────────────────────────
// Rename "Loans Receivable" → "Loan Given" in all display labels
// ─────────────────────────────────────────────────────────────────────────────
function toLoanGivenLabel(accountName, customerName) {
...
async function getLRAccountIds(companyId) {
    ...
    let accounts = await db_1.db.chartOfAccount.findMany({ where, select: { id: true } });
    // Fallback: if no accounts found for specific company, search all companies
    if (accounts.length === 0 && companyId) {
        accounts = await db_1.db.chartOfAccount.findMany({
...
async function listCustomersForCompany(companyId) {
    ...
    let interestAccounts = await db_1.db.chartOfAccount.findMany({
        where: { accountCode: { in: ['4110', '4100', '4001', '4002'] }, ...(companyId ? { companyId } : {}) },
        select: { id: true }
    });
    if (interestAccounts.length === 0 && companyId) {
        interestAccounts = await db_1.db.chartOfAccount.findMany({
    ...
    const journalIds = await db_1.db.journalEntry.findMany({
        where: {
            isReversed: false,
            ...(companyId ? { companyId } : {}),
        },
    ...
    const lines = journalIdSet.length > 0
        ? await db_1.db.journalEntryLine.findMany({
            where: {
    ...
    const mirrorMappings = loanIdsFromLines.length > 0
        ? await db_1.db.mirrorLoanMapping.findMany({
    ...
    const mirrorToOriginalId = new Map(reverseMirrorMappings.filter(m => m.mirrorLoanId).map(m => [m.mirrorLoanId, m.originalLoanId]));
    // 5. Build customer name map from loanId
    const onlineLoans = loanIdsFromLines.length > 0
        ? await db_1.db.loanApplication.findMany({
    ...
    const offlineLoans = loanIdsFromLines.length > 0
        ? await db_1.db.offlineLoan.findMany({
            where: { id: { in: loanIdsFromLines } },
            select: {
                id: true,
                customerName: true,
    ...
    for (const l of onlineLoans) {
        if (l.customer)
            loanToCustomer.set(l.id, { id: l.customer.id, name: l.customer.name || 'Unknown', phone: l.customer.phone || '', email: l.customer.email || '' });
    }
    for (const l of offlineLoans) {
        if (l.customer) {
            loanToCustomer.set(l.id, { id: l.customer.id, name: l.customer.name || l.customerName || 'Unknown', phone: l.customer.phone || l.customerPhone || '', email: l.customer.email || '' });
        }
            ...
            loanToCustomer.set(l.id, { id: syntheticId, name: l.customerName, phone: l.customerPhone || '', email: l.customerEmail || '' });
        }
    }
    ...
    for (const [mirrorId, originalId] of mirrorToOriginalId) {
        if (!loanToCustomer.has(mirrorId) || loanToCustomer.get(mirrorId)?.id.startsWith('offline_')) {
            ...
            if (origCustomer)
                loanToCustomer.set(mirrorId, origCustomer);
        }
    }
    ...
    for (const line of lines) {
        ...
        if (line.loanId) {
            if (mirroredLoanIds.has(line.loanId) && companyId) {
                ...
                if (mirrorCo !== companyId)
                    continue; // Skip — original company view
            }
        }
...
// ─────────────────────────────────────────────────────────────────────────────
async function listCustomersFallback(companyId) {
    const result = [];
    const seenCustomers = new Map();
    const addOrMerge = (customerId, entry) => {
    ...
    const allMirrorMappings = await db_1.db.mirrorLoanMapping.findMany({
    ...
    const calcOutstandingOnline = (emis) => emis.reduce((s, e) => ['PAID', 'WAIVED', 'INTEREST_ONLY_PAID'].includes(e.paymentStatus)
        ? s : s + (e.totalAmount - (e.paidAmount || 0)), 0);
    const calcOutstandingOffline = (emis) => emis.reduce((s, e) => ['PAID', 'WAIVED'].includes(e.paymentStatus)
        ? s : s + (e.totalAmount - (e.paidAmount || 0)), 0);
    const calcPaid = (emis) => emis.reduce((s, e) => s + (e.paidAmount || 0), 0);
    // ══════════════════════════════════════════════════════════════════
    ...
    if (companyId) {
        // --- Online mirror loans ---
        const onlineMirrorMappings = await db_1.db.mirrorLoanMapping.findMany({
        ...
        for (const mapping of onlineMirrorMappings) {
            ...
            const loan = await db_1.db.loanApplication.findUnique({
                where: { id: loanId },
                include: {
                    customer: { select: { id: true, name: true, phone: true, email: true } },
            ...
            if (!loan?.customer) {
                ...
                if (mapping.mirrorLoanId && mapping.mirrorLoanId !== mapping.originalLoanId) {
                    const orig = await db_1.db.loanApplication.findUnique({
        ...
        const offlineMirrorMappings = await db_1.db.mirrorLoanMapping.findMany({
            where: { mirrorCompanyId: companyId, isOfflineLoan: true },
            select: { originalLoanId: true, mirrorLoanId: true }
        });
        for (const mapping of offlineMirrorMappings) {
            const loanId = mapping.mirrorLoanId || mapping.originalLoanId;
            const loan = await db_1.db.offlineLoan.findUnique({
                where: { id: loanId },
                include: {
                    customer: { select: { id: true, name: true, phone: true, email: true } },
    ...
    const allMirroredOnlineIds = new Set((await db_1.db.mirrorLoanMapping.findMany({
        where: { isOfflineLoan: false },
    ...
    if (allMirroredOnlineIds.size > 0)
        onlineWhere.id = { notIn: [...allMirroredOnlineIds] };
    const onlineLoans = await db_1.db.loanApplication.findMany({
        where: onlineWhere,
        include: {
            customer: { select: { id: true, name: true, phone: true, email: true } },
            emiSchedules: { select: { paymentStatus: true, totalAmount: true, paidAmount: true } }
    ...
    const allMirroredOfflineIds = new Set((await db_1.db.mirrorLoanMapping.findMany({
    ...
    if (allMirroredOfflineIds.size > 0)
        offlineWhere.id = { notIn: [...allMirroredOfflineIds] };
    const offlineLoans = await db_1.db.offlineLoan.findMany({
        where: offlineWhere,
        include: {
            customer: { select: { id: true, name: true, phone: true, email: true } },
            emis: { select: { paymentStatus: true, totalAmount: true, paidAmount: true } }
    ...
    for (const loan of offlineLoans) {
        const outstanding = loan.emis.reduce((s, e) => ['PAID', 'WAIVED'].includes(e.paymentStatus)
            ? s : s + (e.totalAmount - (e.paidAmount || 0)), 0);
        ...
        if (loan.customer) {
            ...
            const rawName = loan.customerName || 'Unknown';
            const rawPhone = loan.customerPhone || 'N/A';
...
// ─────────────────────────────────────────────────────────────────────────────
async function getPersonalLedger(customerId, companyId) {
    ...
    if (isNameGroupId && groupName) {
        ...
        const sampleLoan = await db_1.db.offlineLoan.findFirst({
        ...
        const offlineLoan = await db_1.db.offlineLoan.findUnique({
        ...
        customer = await db_1.db.user.findUnique({
            where: { id: customerId },
            select: { id: true, name: true, phone: true, email: true, address: true }
        });
    }
    // Get all loans for this customer
    const allOnlineLoans = isSyntheticId ? [] : await db_1.db.loanApplication.findMany({
    ...
    if (isNameGroupId && groupName) {
        ...
        if (companyId)
            nameWhere.companyId = companyId;
        allOfflineLoans = await db_1.db.offlineLoan.findMany({
            where: nameWhere,
            select: {
    ...
    const mirrorLoanIds = new Set(mirrorMappings.map(m => m.mirrorLoanId).filter(Boolean));
    ...
    for (const mapping of customerMirrorMappings) {
        if (!allOnlineLoans.find(l => l.id === mapping.mirrorLoanId)) {
            const mirrorLoan = await db_1.db.loanApplication.findUnique({
            ...
            if (mirrorLoan)
                extraMirrorOnlineLoans.push({ ...mirrorLoan, _isMirrorRecord: true });
        }
    }
    ...
    for (const mapping of nullMirrorMappings) {
        ...
        if (origLoan)
            extraMirrorOnlineLoans.push({ ...origLoan, _isMirrorRecord: true });
    }
    ...
    const validOnlineLoans = [
        ...
        ...allOnlineLoans.filter(l => {
            if (mirroredIds.has(l.id)) {
                ...
                if (companyId && l.companyId === companyId)
                    return true;
    ...
    if (companyId) {
        ...
        const customerOfflineMirrorMappings = offlineMirrorMappings.filter(m => m.mirrorCompanyId === companyId &&
            m.mirrorLoanId &&
        ...
        for (const mapping of customerOfflineMirrorMappings) {
            if (!allOfflineLoans.find(l => l.id === mapping.mirrorLoanId)) {
                const mirrorLoan = await db_1.db.offlineLoan.findUnique({
                    where: { id: mapping.mirrorLoanId },
                    include: {
                        company: { select: { id: true, name: true } },
                ...
                if (mirrorLoan)
                    extraMirrorOfflineLoans.push(mirrorLoan);
            }
        }
    }
    ...
    if (validLoanIds.length === 0) {
        ...
        return getPersonalLedgerFallback(customerId, companyId, customer, [], allOfflineLoans, mirroredIds);
    }
    ...
    if (allTargetAccountIds.length > 0 && queryLoanIds.length > 0) {
        journalEntries = await db_1.db.journalEntry.findMany({
            where: {
                isReversed: false,
                ...(companyId ? { companyId } : {}),
                lines: {
        ...
        const accrualEntries = await db_1.db.journalEntry.findMany({
            where: {
                isReversed: false,
        ...
        const seenJEIds = new Set(journalEntries.map((je) => je.id));
        ...
        journalEntries.sort((a, b) => new Date(a.entryDate).getTime() - new Date(b.entryDate).getTime());
    }
    ...
    for (const l of validOnlineLoans) {
        loanDataMap.set(l.id, {
            ...
            closedAt: l.closedAt,
            isMirror: mirroredIds.has(l.id) || l._isMirrorRecord,
    ...
    for (const je of journalEntries) {
        ...
        const loanIdsInEntry = Array.from(new Set(rawLoanIds));
        for (let lid of loanIdsInEntry) {
            ...
            if (!entriesByLoan.has(lid))
                entriesByLoan.set(lid, []);
            entriesByLoan.get(lid).push(je);
        }
    }
    // Build loan statement entries (only those from journal entries, only affecting LR)
    const allEntries = [];
    const onlineLoansSummary = [];
    const offlineLoansSummary = [];
    for (const loanId of validLoanIds) {
        const meta = loanDataMap.get(loanId);
        if (!meta)
            continue;
        const loanJEs = entriesByLoan.get(loanId) || [];
        const hasDisbursement = loanJEs.some(je => (je.referenceType === 'LOAN_DISBURSEMENT' || je.referenceType === 'MIRROR_LOAN_DISBURSEMENT') &&
            je.lines.some((l) => lrAccountIds.includes(l.accountId) && l.debitAmount > 0 && l.loanId === loanId));
        ...
        for (const emi of emiList) {
            ...
            const hasRealAccrual = loanJEs.some(je => {
                if (je.referenceType !== 'INTEREST_ACCRUAL' && je.referenceType !== 'INTEREST_RECLASSIFICATION')
                    return false;
                // Match by referenceId (which is emi.id) or by narration matching EMI number
                if (je.referenceId === emi.id)
                    return true;
        ...
        for (const je of loanJEs) {
            ...
            const isLineForThisLoan = (l) => String(l.loanId) === String(loanId) || originalToMirrorLoanId.get(String(l.loanId)) === String(loanId);
            const loanLines = je.lines.filter((l) => isLineForThisLoan(l));
            ...
            let interestPaid = 0;
            if (isPFAccrual) {
                const pfReceivableLines = loanLines.filter((l) => l.account?.accountCode === '1302');
                principalDisbursed = pfReceivableLines.reduce((s, l) => s + l.debitAmount, 0);
            }
            else if (isPFPayment) {
                const pfReceivableLines = loanLines.filter((l) => l.account?.accountCode === '1302');
                interestPaid = pfReceivableLines.reduce((s, l) => s + l.creditAmount, 0);
            }
            else if (isAccrual) {
                // Accruals: Debit column (principalDisbursed) represents interest charged
                const interestReceivableLines = loanLines.filter((l) => ['1301', '1305'].includes(l.account?.accountCode || ''));
                principalDisbursed = interestReceivableLines.reduce((s, l) => s + l.debitAmount, 0);
                if (principalDisbursed === 0) {
                    const interestIncomeLines = loanLines.filter((l) => ['4110', '4100', '4001', '4002'].includes(l.account?.accountCode || ''));
                    principalDisbursed = interestIncomeLines.reduce((s, l) => s + l.creditAmount, 0);
                }
            }
            else if (isPayment) {
                // Payments: Principal paid is credit to 1200/1201/1210. Interest paid is credit to 1301/1305/4110/etc.
                const principalLines = loanLines.filter((l) => ['1200', '1201', '1210'].includes(l.account?.accountCode || ''));
                principalPaid = principalLines.reduce((s, l) => s + l.creditAmount, 0);
                const interestLines = loanLines.filter((l) => ['1301', '1305', '4110', '4100', '4001', '4002'].includes(l.account?.accountCode || ''));
                interestPaid = interestLines.reduce((s, l) => s + l.creditAmount, 0);
            }
            else if (isDisbursement) {
                // Disbursement: Debit column is principal disbursed
                const principalLines = loanLines.filter((l) => ['1200', '1201', '1210'].includes(l.account?.accountCode || ''));
                principalDisbursed = principalLines.reduce((s, l) => s + l.debitAmount, 0);
            }
            else if (isClose) {
                ...
                const lrLines = loanLines.filter((l) => ['1200', '1201', '1210'].includes(l.account?.accountCode || ''));
                const intLines = loanLines.filter((l) => ['1301', '1305', '4110', '4100', '4001', '4002'].includes(l.account?.accountCode || ''));
                principalDisbursed = lrLines.reduce((s, l) => s + l.debitAmount, 0);
                principalPaid = lrLines.reduce((s, l) => s + l.creditAmount, 0);
                interestPaid = intLines.reduce((s, l) => s + l.creditAmount, 0);
            }
        ...
        if (meta.status === 'CLOSED') {
            const hasCloseJE = loanJEs.some(je => je.referenceType === 'PRINCIPAL_ONLY_PAYMENT' ||
            ...
            if (!hasCloseJE) {
                ...
                const closedEMIs = emiList2.filter((e) => e.paymentStatus === 'PAID' || e.paymentStatus === 'WAIVED');
                const closePrincipal = closedEMIs.reduce((s, e) => s + (Number(e.paidPrincipal) || 0), 0);
                const closeInterest = closedEMIs.reduce((s, e) => s + (Number(e.paidInterest) || 0), 0);
                // Find latest paidDate or closedAt
                const latestPaidDates = closedEMIs.map((e) => e.paidDate ? new Date(e.paidDate) : null).filter(Boolean);
        ...
        const isLineForThisLoanSummary = (l) => String(l.loanId) === String(loanId) || originalToMirrorLoanId.get(String(l.loanId)) === String(loanId);
        let totalDisbursed = loanJEsAll.reduce((s, je) => {
            const lrLines = je.lines.filter((l) => ['1200', '1201', '1210'].includes(l.account?.accountCode || '') && isLineForThisLoanSummary(l));
            return s + lrLines.reduce((ss, l) => ss + l.debitAmount, 0);
        }, 0);
        ...
        const totalRepaid = loanJEsAll.reduce((s, je) => {
            ...
            if (!isAnyPayment)
                return s;
            const lrLines = je.lines.filter((l) => ['1200', '1201', '1210'].includes(l.account?.accountCode || '') && isLineForThisLoanSummary(l));
            return s + lrLines.reduce((ss, l) => ss + l.creditAmount, 0);
        }, 0);
        const totalInterestCollected = loanJEsAll.reduce((s, je) => {
            ...
            if (!isAnyPayment)
                return s;
            const iLines = je.lines.filter((l) => ['1301', '1305', '4110', '4100', '4001', '4002'].includes(l.account?.accountCode || '') && isLineForThisLoanSummary(l));
            return s + iLines.reduce((ss, l) => ss + l.creditAmount, 0);
        }, 0);
        ...
        if (meta.loanType === 'ONLINE')
            onlineLoansSummary.push(summary);
        else
            offlineLoansSummary.push(summary);
    }
    allEntries.sort((a, b) => {
...
function buildDescription(refType, narration, emiNumber, loanNumber, customerName) {
    const t = (refType || '').toUpperCase();
    const name = customerName || 'Customer';
    if (t === 'LOAN_DISBURSEMENT' || t === 'MIRROR_LOAN_DISBURSEMENT')
        return `Loan Given - ${name}`;
    if (t === 'EMI_PAYMENT' || t === 'MIRROR_EMI_PAYMENT')
        return `Loan EMI - ${name} (P+I)`;
    if (t === 'INTEREST_ONLY_PAYMENT')
        return emiNumber ? `IO EMI #${emiNumber} - ${name}` : `Interest Only - ${name}`;
    if (t === 'PARTIAL_EMI_PAYMENT')
        return `EMI #${emiNumber || '?'} - Partial Payment - ${name}`;
    if (t === 'PENALTY_COLLECTION')
        return `Late Penalty - ${name}`;
    if (t === 'EXTRA_EMI_PAYMENT')
        return `Extra EMI #${emiNumber || ''} - ${name}`;
    if (t === 'PROCESSING_FEE_ACCRUAL')
        return `Processing Fee Accrued — ${name}`;
    if (t === 'PROCESSING_FEE_COLLECTION' || t === 'PROCESSING_FEE')
        return `Processing Fee Received — ${name}`;
...
// ─────────────────────────────────────────────────────────────────────────────
async function getPersonalLedgerFallback(customerId, companyId, customer, onlineLoans, offlineLoans, mirroredIds) {
    const allEntries = [];
    const onlineLoansSummary = [];
    const offlineLoansSummary = [];
    for (const loan of onlineLoans) {
        const fullLoan = await db_1.db.loanApplication.findUnique({
            where: { id: loan.id },
        ...
        for (const emi of (fullLoan.emiSchedules || [])) {
            ...
            if (!emi.paidDate || emi.paidAmount <= 0)
                continue;
    ...
    for (const loan of offlineLoans) {
        const fullLoan = await db_1.db.offlineLoan.findUnique({
            where: { id: loan.id },
...
// Single loan ledger
// ─────────────────────────────────────────────────────────────────────────────
async function getSingleLoanLedger(loanId, companyId) {
    let loan = await db_1.db.loanApplication.findUnique({
        where: { id: loanId },
        include: {
    ...
    let offlineLoan = null;
    if (!loan) {
        offlineLoan = await db_1.db.offlineLoan.findUnique({
            where: { id: loanId },
            include: {
                customer: { select: { id: true, name: true, phone: true } },
                company: { select: { id: true, name: true } },
        ...
        if (!offlineLoan) {
            return server_1.NextResponse.json({ success: false, error: 'Loan not found' }, { status: 404 });
        }
    }
    ...
    const mirrorMapping = await db_1.db.mirrorLoanMapping.findFirst({
        where: {
            OR: [
    ...
    if (mirrorMapping) {
        if (mirrorMapping.originalLoanId)
            queryLoanIds.push(mirrorMapping.originalLoanId);
        if (mirrorMapping.mirrorLoanId)
            queryLoanIds.push(mirrorMapping.mirrorLoanId);
    }
    // Fetch standard journal entries in the target company
    const standardEntries = lrAccountIds.length > 0 && targetCompanyId
        ? await db_1.db.journalEntry.findMany({
            where: {
                isReversed: false,
    ...
    const hasDisbursement = uniqueJEs.some(je => (je.referenceType === 'LOAN_DISBURSEMENT' || je.referenceType === 'MIRROR_LOAN_DISBURSEMENT') &&
        je.lines.some((l) => lrAccountIds.includes(l.accountId) && l.debitAmount > 0 && queryLoanIds.includes(l.loanId)));
