...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (action === 'disburse') {
      ...
      // ── IO loans: NO EMI schedule until "Start Loan" is clicked (same as original) ──
      if (isIOLoan) {
        // Leave EMI schedule EMPTY — mirror is in ACTIVE_INTEREST_ONLY phase.
        // Schedule will be generated by offline-loan/start or loan/start cascade.
        console.log(`[Mirror Loan IO] No EMI schedule created — mirror is in Interest-Only phase. Monthly interest: ₹${monthlyMirrorInterest}/mo @ ${pendingLoan.mirrorInterestRate}%`);
      } else {
