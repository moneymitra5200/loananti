...
async function triggerNextEMIInterestAccrual(params: {
  ...
  // Return immediately to prevent auto-accruing future EMIs in advance.
  // Future EMIs should only be accrued automatically on or after their due date via daily cron or on-demand accrual,
  // or on-demand during a payment if the payment actually contains interest (FULL_EMI, INTEREST_ONLY, etc.).
  return;
}
