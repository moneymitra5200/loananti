...
import { Bell, AlertTriangle, Clock, CheckCircle, ChevronRight } from 'lucide-react';
...
interface EMIDueAlertBannerProps {
  userId: string;
  userRole: string;
  onAlertClick?: (type: 'overdue' | 'today' | 'tomorrow') => void;
}

export default function EMIDueAlertBanner({ userId, userRole, onAlertClick }: EMIDueAlertBannerProps) {
      ...
      <div className="bg-white px-4 py-3 grid grid-cols-3 gap-3 text-center text-sm">
        {overdue.count > 0 && (
          <button
            ...
            className="p-2 bg-red-50 rounded-lg border border-red-200 animate-pulse-red hover:bg-red-100 transition-colors cursor-pointer group relative"
          >
